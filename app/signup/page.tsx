    ...
    <form action={submit} className="card p-6 max-w-lg mx-auto space-y-3">
      ...
      {turnstileSiteKey ? (
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" onChange={(e) => setCaptchaToken(e.target.checked ? 'dev-captcha-pass' : '')} />
      ...
      <p className="text-sm text-slate-600">Already have an account? <Link href="/signin" className="text-brand underline">Sign in</Link></p>
    </form>
  );
}
